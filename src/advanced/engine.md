...
- La CLI `dockerd` est utilisée pour lancer le Docker daemon
...
- Le Docker client (CLI `docker`) communique avec le daemon via API REST en utilisant la socket `/var/run/docker.sock` par défaut. Le client peut aussi contacter directement une adresse tel que `tcp://127.0.0.1:2375` ou `tcp://198.265.78.1:2376` en fonction des configurations du Daemon
...
Créer un fichier `etc/docker/daemon.json` et y configurer les options suivantes pour le Docker daemon:
...
Cela peut-être utile dans certaines situations, par exemple pour partager un daemon avec une équipe de développeurs ou un système de CI et accélérer le build d'image: le cache de build pourra être réutilisé facilement et ainsi réduire le temps de build (pratique dans certains cas où un build sans cache dure 20+ min et avec cache seulement quelques secondes!)
...
- (Optionnel) Configurer l'authentification du client en TLS - le client devra s'authentifier auprès du serveur avec un certificat pour pouvoir utiliser le daemon
...
Dans le cadre de l'exercice nous pourrons générer nos propres CA et certificats. Pour générer une clé et un certificat autosigné:
